...
class DataGenerator(keras.utils.Sequence):
    ...
    def __data_generation(self, list_IDs_temp):
        ...
        for i, ID in enumerate(list_IDs_temp):
            ...
            sample = img_to_array(load_img(dir_name + ID, color_mode='rgb', interpolation='bilinear', target_size=self.dim), dtype='uint8')
            ...
            sample =   img_to_array(load_img(dir_name + self.labels[ID], color_mode='grayscale', interpolation='bilinear', target_size=self.dim), dtype='uint8')[:,:,0]
            if sample is None:
